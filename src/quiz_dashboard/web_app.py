...
class QuizWebApp:
    ...
    def _register_routes(self) -> None:
        ...
        self.app.route("/quiz/<session_id>/question", methods=["GET"])(
            self.get_next_question
        )
        self.app.route("/quiz/<session_id>/submit", methods=["POST"])(
            self.submit_question_answer
        )
        self.app.route("/quiz/<session_id>/complete", methods=["POST"])(
            self.complete_quiz_session
        )

        # Question management routes
        self.app.route("/questions", methods=["GET"])(self.list_questions)
        self.app.route("/questions/create", methods=["GET", "POST"])(
            self.create_question
        )
        self.app.route("/questions/<int:question_id>", methods=["GET"])(
            self.view_question
        )
        self.app.route("/questions/import", methods=["GET", "POST"])(
            self.import_questions
        )

        # Analytics routes
        self.app.route("/analytics", methods=["GET"])(self.analytics_dashboard)
        self.app.route("/analytics/user/<int:user_id>", methods=["GET"])(
            self.user_analytics
        )

        # API routes
        self.app.route("/api/questions/search", methods=["GET"])(
            self.api_search_questions
        )
        self.app.route("/api/questions/validate", methods=["POST"])(
            self.api_validate_question
        )
        self.app.route("/api/user/<int:user_id>/progress", methods=["GET"])(
            self.api_user_progress
        )
        self.app.route("/api/quiz/recommendations/<int:user_id>", methods=["GET"])(
            self.api_quiz_recommendations
        )
    ...
    def student_dashboard(self) -> str:
        ...
        try:
            ...
            recommended_questions = self.sr_engine.select_next_questions(
                user_id, count=10
            )
    ...
    def complete_quiz_session(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return jsonify(
                {"message": "Quiz completed successfully", "statistics": final_stats}
            )
    ...
    def create_question(self) -> str:
        ...
        try:
            ...
            # Validate question
            validation_result = self.question_manager.validator.validate_question(
                question_data
            )

            if not validation_result.is_valid:
                flash(
                    f"Validation errors: {', '.join(validation_result.errors)}", "error"
                )
                return render_template(
                    "create_question.html",
                    title="Create Question",
    ...
    def import_questions(self) -> str:
        ...
        try:
            ...
            if "questions_file" not in request.files:
                ...
                return render_template(
                    "import_questions.html", title="Import Questions"
                )
            ...
            try:
                ...
                # Display results
                flash(
                    f"Import completed: {import_result['imported']}/{import_result['total']} questions imported",
                ...
                if import_result["warnings"]:
                    flash(
                        f"Warnings: {'; '.join(import_result['warnings'][:5])}",
                        "warning",
                    )
...
>>>>>>> 09358808f3e803617735c925d71ec563c5edd780
        ...
        correct_pairs = {
            (match["left_id"], match["right_id"]) for match in correct_matches
        }
        user_pairs = set()
...
>>>>>>> 09358808f3e803617735c925d71ec563c5edd780
        ...
        question_data = {
            ...
            "hashtags": [
                tag.strip()
                for tag in form_data.get("hashtags", "").split(",")
                if tag.strip()
            ],
        }
        ...
        if question_type == QuestionType.MULTIPLE_CHOICE.value:
            ...
            for i, text in enumerate(option_texts):
                if text.strip():
                    options.append(
                        {
                            "id": (
                                option_ids[i] if i < len(option_ids) else f"option_{i}"
                            ),
                            "text": text.strip(),
                        }
                    )
            ...
            acceptable_answers = [
                ans.strip()
                for ans in form_data.get("acceptable_answers", "").split("\n")
                if ans.strip()
            ]
    ...
    def _get_question_type_statistics(self) -> Dict[str, int]:
        ...
        try:
            # Query to get count of questions by type
            query = """
            ...
            # Return dictionary mapping question types to counts
            return {row["question_type"]: row["count"] for row in results}

        except Exception as e:
            # Log error and return empty dictionary on failure
    ...
    def _get_system_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Query and collect question statistics
            ...
            # Add question statistics to results if available
            if question_stats:
                stats.update(
                    {
                        ...
                        "avg_difficulty": round(
                            question_stats[0]["avg_difficulty"] or 0, 2
                        ),
                    }
                )

            # Query and collect user statistics
            ...
            # Add user statistics to results if available
            if user_stats:
                stats.update(
                    {
            ...
            stats = {
                ...
                "accuracy_percentage": round(
                    (
                        ...
                        / max(session_data["questions_answered"], 1)
                    )
                    * 100,
                    2,
                ),
                "completion_percentage": round(
                    (
                        ...
                        / max(session_data["total_questions"], 1)
                    )
                    * 100,
                    2,
                ),
