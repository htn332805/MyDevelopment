#!/usr/bin/env python3
"""
Write the team Copilot instructions file into the repository.
...
def main(argv: list[str] | None = None) -> int:
    ...
    """
    parser = argparse.ArgumentParser(description="Write Copilot instructions file for this repo.")
    parser.add_argument(
        "output",
        nargs="?",
        default=".github/copilot-instructions.md",
        help="Output markdown path (default: .github/copilot-instructions.md)",
    )
    args = parser.parse_args(argv)

    out_path = Path(args.output)
    write_instructions(out_path)
    print(f"Wrote Copilot instructions to: {out_path}")
    return 0
